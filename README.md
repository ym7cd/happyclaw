...
HappyClaw 是一个以 Claude 为主线、同时支持 Codex 的自托管多用户 AI Agent 系统。它将完整的 Claude Code 运行时与独立的 Codex runner 封装为可通过飞书、Telegram、QQ 和 Web 界面访问的服务，支持文件读写、终端操作、浏览器自动化、多轮推理及 MCP 工具生态。
...
2. **配置 Claude / Codex** — 在设置页分别填写 Claude provider 或 Codex 凭证
